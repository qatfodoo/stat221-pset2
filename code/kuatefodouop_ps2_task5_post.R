...
files.param <- sapply(1:4, function(i) {
  list.files(path = "./out/task5", pattern = paste("param" ,i, "+", sep=""))
}, simplify=F)
...
for (p in 1:n.param) {
  ...
  for (t in 1:3) {
    ...
    load(paste("./out/task5/", files.param[[p]][t], sep=""))
...
for (p in 1:n.param) {
  ...
  p_logt68.list[[p]] <- ggplot(, aes(x=as.vector(ltheta), as.vector(cov68))) + 
    ...
    ylab("cov_68") +
    xlab("log.theta") +
    labs(title="68% coverage against Log Theta, task 5")
  ...
  p_logt95.list[[p]] <- ggplot(, aes(x=as.vector(ltheta), as.vector(cov95))) + 
    ...
    ylab("cov_95") +
    xlab("log.theta") +
    labs(title="95% coverage against Log Theta, task 5")
  ...
  p_logw68.list[[p]] <- ggplot(, aes(x=as.vector(logw), as.vector(cov68))) + 
    ...
    ylab("cov_68") +
    xlab("log.w") +
    labs(title="68% coverage against Log w, task 5")
  ...
  p_logw95.list[[p]] <- ggplot(, aes(x=as.vector(logw), as.vector(cov68))) + 
    ...
    ylab("cov_95") +
    xlab("log.w") +
    labs(title="95% coverage against Log w, task 5")
